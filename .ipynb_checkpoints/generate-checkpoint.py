...
from im2mesh.utils.visualize import visualize_data, visualize_mesh, visualize_pointcloud
...
# Dataset
dataset = config.get_dataset('viz', cfg, return_idx=True)
...
for it, data in enumerate(tqdm(test_loader)):
    ...
    if generate_mesh:
        ...
        mesh_img_out_file = os.path.join(mesh_dir, '%s.png' % modelname)
        input_img_out_file = os.path.join(mesh_dir, '%s_in.png' % modelname)
        mesh.export(mesh_out_file)
        visualize_mesh(mesh, mesh_img_out_file)
        visualize_pointcloud(data['inputs'][0].cpu(),out_file=input_img_out_file)
